...
export function printProgress(stats: ScrapingStats): void {
  ...
  if (
    typeof stats.backfillInserted === 'number' ||
    typeof stats.backfillUpdated === 'number' ||
    typeof stats.backfillPages === 'number' ||
    typeof stats.backfillUsers === 'number'
  ) {
    message.push(
      chalk.white(
        `${EMOJI_INFO} Backfill Inserts: ${stats.backfillInserted ?? 0}`
      ),
      chalk.white(
        `${EMOJI_INFO} Backfill Updates: ${stats.backfillUpdated ?? 0}`
      ),
      chalk.white(
        `${EMOJI_INFO} Backfill Pages: ${stats.backfillPages ?? 0}`
      ),
      chalk.white(
        `${EMOJI_INFO} Backfill Users: ${stats.backfillUsers ?? 0}`
      )
    )
  }
