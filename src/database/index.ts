...
function ensureBackfillTable(): void {
  const currentDB = getDatabase()
  currentDB.exec(`
    CREATE TABLE IF NOT EXISTS backfill_progress (
      run_id TEXT PRIMARY KEY,
      inserted_posts INTEGER NOT NULL,
  ...
  // If existing table is from an older schema without run_id (or other columns), rebuild it
  const info = currentDB
    .prepare("PRAGMA table_info('backfill_progress')")
    .all() as { name: string }[]
  ...
  const hasAll = required.every((c) => names.has(c))
  if (!hasAll) {
    currentDB.exec('DROP TABLE IF EXISTS backfill_progress')
    currentDB.exec(`
...
export async function getPostsByThread(threadUrl: string): Promise<Post[]> {
  const currentDB = getDatabase()
  const stmt = currentDB.prepare(
    'SELECT id, thread_url as threadUrl, username, comment, posted_at as postedAt, user_url as userUrl, forum_post_id as forumPostId, post_url as postUrl FROM posts WHERE thread_url = ? ORDER BY posted_at ASC'
  )
...
export function insertPost(
  ...
  const currentDB = getDatabase()
  try {
    const isBackfill = config.BACKFILL_MODE
    const stmt = isBackfill
      ? currentDB.prepare(
          'INSERT INTO posts (thread_url, username, comment, posted_at, user_url, forum_post_id, post_url) VALUES (?, ?, ?, ?, ?, ?, ?) ON CONFLICT(thread_url, forum_post_id) DO UPDATE SET username=excluded.username, comment=excluded.comment, posted_at=excluded.posted_at, user_url=excluded.user_url, post_url=COALESCE(excluded.post_url, posts.post_url)'
        )
      : currentDB.prepare(
          'INSERT OR IGNORE INTO posts (thread_url, username, comment, posted_at, user_url, forum_post_id, post_url) VALUES (?, ?, ?, ?, ?, ?, ?)'
        )
    ...
    if (!isBackfill && typeof result.lastInsertRowid === 'number' && result.changes > 0) {
...
export function startBackfillRun(runId: string): void {
  const currentDB = getDatabase()
  ensureBackfillTable()
  try {
    currentDB
      .prepare(
        `INSERT OR REPLACE INTO backfill_progress (run_id, inserted_posts, updated_posts, pages_processed, started_at, last_updated, finished_at)
         VALUES (?, 0, 0, 0, datetime('now'), datetime('now'), NULL)`
      )
      .run(runId)
  } catch (error) {
    logError(`${EMOJI_ERROR} Failed to start backfill run`, error as Error)
  }
}

export function updateBackfillRun(
  runId: string,
  inserted: number,
  updated: number,
  pages: number
  ...
  try {
    currentDB
      ...
      .run(inserted, updated, pages, runId)
  } catch (error) {
    logError(`${EMOJI_ERROR} Failed to update backfill run`, error as Error)
  }
}

export function finishBackfillRun(runId: string): void {
  const currentDB = getDatabase()
  try {
    currentDB
      .prepare(
    ...
    logError(`${EMOJI_ERROR} Failed to finish backfill run`, error as Error)
  }
}
