...
let currentBackfillRunId: string | null = null
  ...
  for (let attempt = 1; attempt <= config.MAX_RETRIES; attempt++) {
    try {
      ...
      if (config.USE_PLAYWRIGHT_FETCH) {
        let pwAttempt = 0
        let pwHtml = ''
        while (pwAttempt < config.MAX_RETRIES) {
          pwAttempt++
          simpleLogInfo(
            `Fetching via Playwright: ${url} (Attempt ${pwAttempt}/${config.MAX_RETRIES})`
          )
          pwHtml = await fetchViaPlaywright(url)
          if (pwHtml && pwHtml.length > 0 && !isBotChallengePage(pwHtml)) {
          ...
          logWarning(
            `Playwright returned empty or challenge HTML for ${url} (attempt ${pwAttempt}); retrying after wait`
          )
          await delay(config.PLAYWRIGHT_WAIT_MS || 3000)
        }
        throw createFetchError(
          'stuck',
          `Playwright could not bypass challenge for ${url}`
        )
      }
      const urlObj = new URL(url)
...
async function scrapeThreadPosts(
  ...
  backfillRunId: string | null = null
): Promise<void> {
  ...
  while (pageUrl) {
    ...
    try {
      ...
      while (attempt <= maxPageRetries && !pageSucceeded) {
          ...
          try {
            ...
            const postUrl = relativePostUrl
              ...
              : postId
                ...
                : null
            const postTitle =
              $post.find('h2.title').first().text().trim() || ''
            const comment =
              postTitle && baseComment
                ? `${postTitle}\n\n${baseComment}`
                : postTitle || baseComment

            const existed = !!(postId && knownPostIds.has(postId))
            if (existed && !config.BACKFILL_MODE) {
              duplicatePosts++
              continue
            }
            ...
            if (username && comment && derivedUserUrl && postId) {
              ...
              if (config.BACKFILL_MODE) {
                if (existed) {
                  stats.backfillUpdated = (stats.backfillUpdated ?? 0) + 1
                  updatedPosts++
                } else {
                  stats.backfillInserted = (stats.backfillInserted ?? 0) + 1
                }
                stats.backfillUsers = allUsers.size
              }
        ...
        simpleLogInfo(
          `Thread page ${currentPageNumber} posts: inserted=${insertedPosts}, updated=${updatedPosts}, duplicates=${duplicatePosts}`
        )
        ...
        if (config.BACKFILL_MODE) {
          stats.backfillPages = (stats.backfillPages ?? 0) + 1
          if (backfillRunId) {
            updateBackfillRun(
              backfillRunId,
              stats.backfillInserted ?? 0,
              stats.backfillUpdated ?? 0,
              stats.backfillPages ?? 0
            )
          }
        }
        // Only advance checkpoint if we're moving forward and not running in backfill mode
        if (pageUrl && !config.BACKFILL_MODE) {
...
async function scrapeSingleThread(
  ...
  let resumePageNumber = 1
  let resumePageUrl: string | null = null

  if (config.BACKFILL_MODE) {
    simpleLogInfo('Backfill mode enabled; starting from first page')
  } else {
    ...
    const postsBasedPageNumber = Math.max(
      1,
      Math.ceil(existingPostCount / DEFAULT_POSTS_PER_PAGE) -
        RESUME_LOOKBACK_PAGES
    )
    resumePageNumber = Math.max(savedPageNumber, postsBasedPageNumber)
    resumePageUrl =
...
async function main() {
  ...
  try {
    ...
    if (config.BACKFILL_MODE) {
      ...
      stats.backfillUpdated = 0
      stats.backfillPages = 0
      currentBackfillRunId = `backfill-${Date.now()}`
      startBackfillRun(currentBackfillRunId)
    } else {
      currentBackfillRunId = null
    }
    ...
    for (let i = startIndex; i < subforums.length; i++) {
      ...
      for (let j = threadStartIndex; j < threads.length; j++) {
        ...
        await scrapeThreadPosts(thread.url, allUsers, undefined, undefined, new Set(), 0, currentBackfillRunId)
        await delay(config.DELAY_BETWEEN_REQUESTS)
      }
    ...
    if (config.BACKFILL_MODE && currentBackfillRunId) {
      updateBackfillRun(
        currentBackfillRunId,
        stats.backfillInserted ?? 0,
        stats.backfillUpdated ?? 0,
      ...
      finishBackfillRun(currentBackfillRunId)
      currentBackfillRunId = null
    }
